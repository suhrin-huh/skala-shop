...
### 잠금 순서: 항상 `Project` → `Customer`

이 순서를 서비스 전역에서 지킵니다. 후원·취소·삭제·정산이 모두 같은 순서로 잠가야 데드락 사이클이 생기지 않습니다.

| 대상 | 왜 잠그는가 |
|---|---|
| `Project` (`findByIdWithLock`) | `currentAmount` / `pledgeCount` 는 **읽고-더하고-쓰는** 갱신이다. 서로 다른 회원이 같은 프로젝트에 동시 후원하면 Customer 락만으로는 **갱신이 유실**된다 (실제로 하네스가 잡아낸 버그다). |
| `Customer` (`findByIdWithLock`) | 같은 회원의 동시 요청이 같은 사용 가능 포인트를 읽고 초과 후원하는 것을 막는다. |

- 같은 종류의 행을 여러 개 잠글 때는 **id 오름차순**으로 (정산 배치가 여러 후원자를 잠글 때).
- 프로젝트 단위 직렬화이므로 인기 프로젝트의 후원 처리량이 제한됩니다. 정합성 최우선 정책에 따른 의도적 선택입니다.
- 완료 조건: 동일 계정 동시 후원 시 `reservedPoint <= point` 가 항상 유지되고, 비정규화 값이 실제 SUM 과 일치함을 테스트로 검증. → `harness-verify` 스킬 참조.
